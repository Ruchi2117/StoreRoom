"""Analyze saved class_confidence runs without further inference."""
import json
...
def main():
    verify_protected()
    a,b=[json.loads((ROOT/f'reports/class_confidence_001_{t}.json').read_text()) for t in ['a','b']]
    old=json.loads((ROOT/'reports/confidence_001_a.json').read_text())
    ...
    assert replay, 'Confidence .25 must reproduce RESOLUTION_001 A'
    ...
    cohorts={
        ...
        'zero_predictions_with_targets_at_025':[i for i,r in enumerate(a['images']) if sum(r['truth'])>0 and sum(r['predicted'])==0],
        'zero_true_positives_with_targets_at_025':[i for i,r in enumerate(a['images']) if sum(r['truth'])>0 and not r['matching']['matched_gt_indices']]}
    cohort_results={}
    ...
    result={'a_reproduces_confidence_001_a':replay,'images':comparisons,'cohorts':cohort_results,'identity_errors':identities,'small_targets':small,
    ...
    added_kinds=Counter()
    transitions=[]
    for x,y,comp in zip(a['images'],b['images'],comparisons):
        dx={d['prediction_index']:d for d in x['matching']['predictions']}
        dy={d['prediction_index']:d for d in y['matching']['predictions']}
        for j in comp['added_b_indices']:
            added_kinds[dy[j]['kind']]+=1
        for i,j in comp['kept_indices']:
            if dx[i]['kind']!=dy[j]['kind']:
                transitions.append({'image':x['image'],'a_index':i,'b_index':j,'before':dx[i]['kind'],'after':dy[j]['kind']})
    result['added_prediction_kinds']=dict(added_kinds)
    result['retained_prediction_status_changes']=transitions
    result['non_red_bull_predictions_unchanged']=all(len([d for d in x['detections'] if d['class_id']!=0])==len([d for d in y['detections'] if d['class_id']!=0]) and all(same_prediction(p,q) for p,q in zip([d for d in x['detections'] if d['class_id']!=0],[d for d in y['detections'] if d['class_id']!=0])) for x,y in zip(a['images'],b['images']))
    result['removed_predictions']=sum(len(r['removed']) for r in comparisons)
    save(ROOT/'reports/class_confidence_001_comparison.json',result)
    ...
    for x,y in zip(a['images'],b['images']):
        ...
        for j,(title,r) in enumerate([('All classes .25',x),('Red Bull .15 / others .25',y)],1):
            canvas.paste(panel(path,[(d['class_id'],*d['xyxy']) for d in r['detections']],title+' | 320px / NMS .50'),(620*j,0))
        ImageDraw.Draw(canvas).text((10,640),f"Truth {x['truth']} | .25 {x['predicted']} | RB .15 {y['predicted']} | Red Bull / Knoppers / Classic / Still / Capri-Sun",fill='white',font=ImageFont.load_default(size=18))
        canvas.save(ROOT/'reports/visuals'/f"class_confidence_001_{Path(x['image']).stem}.jpg",quality=92)
    verify_protected()
