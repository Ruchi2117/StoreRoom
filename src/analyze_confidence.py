"""Analyze saved confidence runs without further inference."""
import json
...
def main():
    verify_protected()
    a,b=[json.loads((ROOT/f'reports/confidence_001_{t}.json').read_text()) for t in ['a','b']]
    old=json.loads((ROOT/'reports/resolution_001_a.json').read_text())
    ...
    result={'a_reproduces_resolution_001_a':replay,'images':comparisons,'cohorts':cohort_results,'identity_errors':identities,'small_targets':small,
    ...
    save(ROOT/'reports/confidence_001_comparison.json',result)
    ...
    for x,y in zip(a['images'],b['images']):
        ...
        for j,(title,r) in enumerate([('Confidence 0.25',x),('Confidence 0.15',y)],1):
            canvas.paste(panel(path,[(d['class_id'],*d['xyxy']) for d in r['detections']],title+' | 320px / NMS .50'),(620*j,0))
        ImageDraw.Draw(canvas).text((10,640),f"Truth {x['truth']} | .25 {x['predicted']} | .15 {y['predicted']} | Red Bull / Knoppers / Classic / Still / Capri-Sun",fill='white',font=ImageFont.load_default(size=18))
        canvas.save(ROOT/'reports/visuals'/f"confidence_001_{Path(x['image']).stem}.jpg",quality=92)
    verify_protected()
