...
def main():
    verify_protected()
    a,b=[json.loads((ROOT/f'reports/resolution_001_{t}.json').read_text()) for t in ['a','b']]
    old=json.loads((ROOT/'reports/nms_001_b.json').read_text())
    ...
    assert replay, '320 must reproduce NMS_001 B'
    ...
    result={'a_reproduces_nms_001_b':replay,'images':comparisons,'cohorts':cohort_results,'identity_errors':identities,'small_targets':small,
    ...
    save(ROOT/'reports/resolution_001_comparison.json',result)
    ...
    for x,y in zip(a['images'],b['images']):
        ...
        for j,(title,r) in enumerate([('320px',x),('640px',y)],1):
            canvas.paste(panel(path,[(d['class_id'],*d['xyxy']) for d in r['detections']],title+' | conf .25 / NMS .50'),(620*j,0))
        ImageDraw.Draw(canvas).text((10,640),f"Truth {x['truth']} | 320 {x['predicted']} | 640 {y['predicted']} | Red Bull / Knoppers / Classic / Still / Capri-Sun",fill='white',font=ImageFont.load_default(size=18))
        canvas.save(ROOT/'reports/visuals'/f"resolution_001_{Path(x['image']).stem}.jpg",quality=92)
    verify_protected()
