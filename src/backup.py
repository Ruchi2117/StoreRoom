    ...
    try:
        with closing(connect_readonly(database)) as connection:
            if connection.execute('PRAGMA user_version').fetchone()[0] != 1:
                raise BackupError('Only migrated schema version 1 is supported')
            ...
            if {n for n, t in objects if t == 'table'} != set(Base.metadata.tables) or any(t in {'view', 'trigger'} for _, t in objects):
...
def unpack_validated(archive, target):
    ...
    try:
        with zipfile.ZipFile(archive) as zipped:
            ...
            if manifest['format_version'] != 1 or manifest['schema_version'] != 1:
...
class BackupService:
    ...
    def create_backup(self, destination):
        ...
        with data_lock(self.database, self.scans), tempfile.TemporaryDirectory(
            ...
            manifest = {'format_version': 1, 'application_version': '0.6', 'schema_version': 1,
