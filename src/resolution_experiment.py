...
from src.nms_experiment import ROOT, save, protect, match_detections, summarize
...
def verify_protected():
    protect()
    snapshot = json.loads((ROOT/'reports/resolution_001_preservation.json').read_text())
...
def main():
    setup_path = ROOT/'reports/resolution_001_setup.json'
    if setup_path.exists():
        raise FileExistsError('RESOLUTION_001 already started; refusing to overwrite')
    protect()
    ...
    for folder in ['reports', 'src', 'configs', 'tests']:
        for p in (ROOT/folder).rglob('*'):
            if p.is_file() and '__pycache__' not in p.parts and 'resolution' not in p.name.lower():
                snapshot[p.relative_to(ROOT).as_posix()] = digest(p)
    save(ROOT/'reports/resolution_001_preservation.json', snapshot)
    ...
    setup['common_arguments'].pop('imgsz')
    setup['common_arguments']['iou'] = .50
    setup['runs'] = {'A':320, 'B':640}
    ...
    for tag,size in setup['runs'].items():
        started = time.perf_counter()
        model = YOLO(setup['checkpoint'])
        outputs = []
        for row in rows:
            path = ROOT/'data/yolo_v01/images/val'/row['image']
            pred = model.predict(str(path), imgsz=size, **setup['common_arguments'])[0]
            boxes = pred.boxes
        ...
        report = {'status':'completed','run':tag,'imgsz':size,'confidence':.25,'nms_iou':.5,
        ...
        save(ROOT/f'reports/resolution_001_{tag.lower()}.json',report)
        verify_protected()
