...
class ClassConfidenceChecks(unittest.TestCase):
    def test_frozen_validation_protocol_and_metrics(self):
        ...
        self.assertEqual(setup['runs'],{'A':[.25]*5,'B':[.15,.25,.25,.25,.25]})
        split=json.loads((ROOT/'configs/splits.json').read_text())['images']
        for tag,thresholds in [('a',[.25]*5),('b',[.15,.25,.25,.25,.25])]:
            run=json.loads((ROOT/f'reports/class_confidence_001_{tag}.json').read_text())
            self.assertEqual(run['imgsz'],320)
            self.assertEqual(run['class_thresholds'],thresholds)
        ...
        comparison=json.loads((ROOT/'reports/class_confidence_001_comparison.json').read_text())
        self.assertTrue(comparison['a_reproduces_confidence_001_a'])
        self.assertTrue(comparison['non_red_bull_predictions_unchanged'])
        self.assertEqual(sum(comparison[k] for k in ['improved','unchanged','worsened']),26)

    def test_pre_nms_gate_sanity(self):
        from src.class_confidence_predictor import sanity_check
        self.assertTrue(sanity_check()['passed'])
