...
class InferenceTests(unittest.TestCase):
    ...
    def test_http_uses_same_detector_and_schema(self):
        ...
        with TestClient(create_app(detector_factory=factory,output_dir=detector.output_dir,database_path=self.directory/"test.db",scan_storage_dir=self.directory/"scans")) as client:
            self.assertEqual(client.get('/health').json(),{'status':'ok'})
            response=client.post('/predict?annotate=false',files={'file':('shelf.png',image_bytes(),'image/png')})
            self.assertEqual(response.status_code,200,response.text)
            payload=response.json()
            self.assertTrue(payload.pop('prediction_id'))
    ...
    def test_http_invalid_and_multiple_uploads(self):
        detector=self.detector()
        with TestClient(create_app(detector_factory=lambda **_:detector,database_path=self.directory/"test.db",scan_storage_dir=self.directory/"scans")) as client:
